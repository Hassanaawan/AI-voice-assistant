...
def get_intent(user_input):
    try:
        ...
        # Greeting manual check
        for key in greeting_responses:
            if key in user_input_clean:
                return {
                    "intent": "greeting",
                    "confidence": 1.0,
                    "response": greeting_responses[key] + " " + ASSISTANT_INTRO # Combine into a single string
                }
        ...
        if not seq or not seq[0]:
            return {
                "intent": "unknown",
                "confidence": 0.0,
                "response": "Sorry, I couldn't understand that. Please rephrase." # Changed to single string
            }
        ...
        if confidence >= CONFIDENCE_THRESHOLD:
            ...
            return {"intent": intent_label, "confidence": confidence, "response": response_message} # Already a single string
        else:
            return {
                "intent": "unknown",
                "confidence": confidence,
                "response": "I'm not sure I understand. Could you rephrase?" # Changed to single string
            }
        ...
        return {
            "intent": "error",
            "confidence": 0.0,
            "response": f"An error occurred during prediction: {str(ex)}" # Already a single string
        }
